...
from src.grid import Grid, create_simple_grid, create_8_node_complex_grid, create_20_node_large_grid
from src.optimization import run_optimization, initialize_optimization
...
def run_demo(grid_type: str = "medium"):
    ...
    plot_grid(grid, state_init, f"Initial State ({grid_type})", f"initial_grid{suffix}.png")
    ...
    print(f"Max Line Capacity Violation: {jnp.max(jnp.maximum(0.0, jnp.abs(flows) - grid.line_capacity)):.6f}")
    
    print("Saving Final State Plot...")
    plot_grid(grid, final_state, f"Optimized State ({grid_type})", f"final_grid{suffix}.png")
...
def main():
    ...
    parser.add_argument("--mode", choices=["simple", "medium", "complex", "large", "all"], default="medium",
