...
def create_8_node_complex_grid() -> Grid:
    ...
    line_capacity = jnp.array([
        80.0, 30.0, 60.0, 80.0, 40.0, 50.0, # Source lines
        30.0, 30.0, 30.0, 30.0, 30.0        # Mesh connections
    ])
...
def create_20_node_large_grid(seed: int = 42) -> Grid:
    ...
    node_is_generator = jnp.zeros(20, dtype=bool).at[jnp.array([0, 5, 10, 15])].set(True)
    ...
    # Gen 10: Mid
    # Gen 15: Expensive
    gen_cost_a = jnp.zeros(20).at[jnp.array([0, 5, 10, 15])].set(jnp.array([0.002, 0.01, 0.01, 0.04]))
    gen_cost_b = jnp.zeros(20).at[jnp.array([0, 5, 10, 15])].set(jnp.array([5.0, 15.0, 20.0, 50.0]))
